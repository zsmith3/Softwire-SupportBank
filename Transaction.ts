...
export class Transaction {
    ...
    toJSON(): JsonRecord {
        return {
            Date: this.date.toFormat("yyyy-MM-dd'T'HH:mm:ss"),
            FromAccount: this.from.name,
            ToAccount: this.to.name,
    ...
    toXMLFormat(): XmlRecord {
        return {
            ["@_Date"]: this.date.diff(DateTime.fromObject({year: 1900}), "days").days.toString(),
            Description: this.narrative,
            Value: this.amount.toString(),
            Parties: {From: this.from.name, To: this.to.name}
        };
    }
